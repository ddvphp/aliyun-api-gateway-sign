 ...
 * under the License.
 */
namespace DdvPhp\AliyunApiGatewaySign\Util;

use DdvPhp\AliyunApiGatewaySign\Http\Constants;
use DdvPhp\AliyunApiGatewaySign\Http\Header;
use DdvPhp\AliyunApiGatewaySign\Http\SystemHeader;
...
class Sign
{
	...
	private static function BuildStringToSign($path, $method, &$headers, $querys, $bodys, $signHeaderPrefixList)
    {
		...
		$sb.= Constants::LF;
		if (array_key_exists(Header::HTTP_HEADER_ACCEPT, $headers) && null != $headers[Header::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[Header::HTTP_HEADER_ACCEPT];
		}
		$sb.= Constants::LF;
		if (array_key_exists(Header::HTTP_HEADER_CONTENT_MD5, $headers) && null != $headers[Header::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[Header::HTTP_HEADER_CONTENT_MD5];
		}
		$sb.= Constants::LF;
		if (array_key_exists(Header::HTTP_HEADER_CONTENT_TYPE, $headers) && null != $headers[Header::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[Header::HTTP_HEADER_CONTENT_TYPE];
		}
		$sb.= Constants::LF;
		if (array_key_exists(Header::HTTP_HEADER_DATE, $headers) && null != $headers[Header::HTTP_HEADER_ACCEPT]) {
			$sb.= $headers[Header::HTTP_HEADER_DATE];
		}
		$sb.= Constants::LF;		
    ...
    */
    private static function BuildHeaders(&$headers, $signHeaderPrefixList)
	{
		...
		if (null != $signHeaderPrefixList)
		{
			...
			unset($signHeaderPrefixList[Header::HTTP_HEADER_ACCEPT]);
			unset($signHeaderPrefixList[Header::HTTP_HEADER_CONTENT_MD5]);
			unset($signHeaderPrefixList[Header::HTTP_HEADER_CONTENT_TYPE]);
			unset($signHeaderPrefixList[Header::HTTP_HEADER_DATE]);
